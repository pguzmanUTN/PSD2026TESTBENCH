...
arr = np.array([tiempo1, tension1]).T
np.savetxt('AM2.csv', arr, delimiter=',')
...
tensionmod = np.abs(tension1)

tensionmax = np.max(tensionmod)

tensiomean = np.mean(tensionmod)

Spp= tensiomean*np.pi/2

m= tensionmax/Spp - 1
print('tensiomean %0.5f'%tensiomean)
print('m = %0.5f'%m)
